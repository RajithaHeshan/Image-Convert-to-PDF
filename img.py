...
class ImageToPDFConverter:
   ...
   def initialize_ui(self):
      ...
      select_images_button = tk.Button(self.root, text="Select Images",  # image select
      command=self.select_images)
      select_images_button.pack(pady=(0,10))

      self.selected_images_listbox.pack (pady=(0,10), fill=tk.BOTH, expand=True)    # select images show in listbox

      lable = tk.Label(self.root, text="Enter output PDF name:")
      lable.pack()
      
      pdf_name_entry = tk.Entry(self.root, textvariable=self.output_pdf_name, width=40, justify='center')  # enter pdf name
      pdf_name_entry.pack()

      convert_button = tk.Button(self.root, text="Convert to PDF",   # convert button
   ...
   def update_selected_images_listbox(self):
       self.selected_images_listbox.delete(0, tk.END)    # tk  define view and previouse selected images deleted
   ...
   def convert_images_to_pdf(self):
      ...
      #selected image add pdf one by one

      for image_path in self.image_paths:
         img = Image.open(image_path)
         available_width = 540     # pdf image size
